...
class WebhookRouteTests(unittest.TestCase):
    ...
    def test_webhook_token_mode_prefetches_lead_id_lookups(self) -> None:
        lead = self._create_lead()
    ...
    def test_webhook_token_mode_resolves_lead_by_website_domain_with_port(self) -> None:
        ...
        lead = Lead(
            id=uuid4(),
            name="Domain Match Port",
            category="HVAC",
            source="google_places",
            website_url="https://acme.example",
            website_domain="Acme.Example:443",
            status="Discovered",
        )
        self.db.add(lead)
        self.db.commit()
        ...
        response = self.client.post(
            "/webhooks/outreach-events",
            headers={"X-Webhook-Token": "test_shared_secret"},
            json={"events": [{"email_or_domain": "acme.example", "event_type": "replied", "event_id": "evt-domain-port-1"}]},
        )
        self.assertEqual(response.status_code, 200, response.text)
        body = response.json()
        self.assertEqual(body["processed"], 1)
    ...
    def test_webhook_token_mode_resolves_lead_by_website_url_with_port(self) -> None:
        from app.models import Lead
        ...
        lead = Lead(
            ...
            category="HVAC",
            source="google_places",
            website_url="https://acme.example:443/path",
            website_domain=None,
            status="Discovered",
        )
        self.db.add(lead)
        ...
        response = self.client.post(
            "/webhooks/outreach-events",
            headers={"X-Webhook-Token": "test_shared_secret"},
            json={"events": [{"email_or_domain": "acme.example", "event_type": "replied", "event_id": "evt-url-port-1"}]},
        )
        self.assertEqual(response.status_code, 200, response.text)
        body = response.json()
        self.assertEqual(body["processed"], 1)
    ...
    def test_webhook_token_mode_blank_event_id_normalizes_to_none(self) -> None:
        ...
        lead = self._create_lead()
        payload = {"events": [{"lead_id": str(lead.id), "event_type": "replied", "event_id": "   "}]}
        headers = {"X-Webhook-Token": "test_shared_secret"}
        ...
        self.assertEqual(second.json()["duplicates"], 0)

        events = self.db.execute(select(OutreachEvent)).scalars().all()
        self.assertEqual(len(events), 2)
        self.assertTrue(all(event.external_id is None for event in events))
