...
class MetricsRouteTests(unittest.TestCase):
    ...
    def test_metrics_summary_provider_filter_falls_back_to_payload_provider_when_column_missing(self) -> None:
        from app.models import Lead, OutreachEvent
        ...
        self.db.add(
            OutreachEvent(
                ...
                provider=None,
                created_at=now,
                payload={"provider": "  SendGrid  ", "provider_event_id": "sg-payload-provider-1"},
            )
        )
        self.db.commit()
        ...
        self.assertEqual(body["webhook_event_types_by_provider_today"], {"sendgrid": {"bounced": 1}})
        self.assertEqual(body["latest_webhook_providers"], ["sendgrid"])
        self.assertEqual(body["latest_event_types_for_provider"], ["bounced"])

    def test_metrics_summary_provider_filter_falls_back_to_payload_provider_when_column_blank(self) -> None:
        from app.models import Lead, OutreachEvent
        ...
        self.db.add(
            OutreachEvent(
                ...
                provider="   ",
                created_at=now,
                payload={"provider": "  SendGrid  ", "provider_event_id": "sg-payload-provider-blank-1"},
            )
        )
        self.db.commit()
