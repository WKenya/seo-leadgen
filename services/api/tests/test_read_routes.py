...
class ReadRouteTests(unittest.TestCase):
    ...
    def test_list_events_provider_filter_falls_back_to_payload_provider_when_column_blank(self) -> None:
        from app.models import OutreachEvent
        ...
        legacy_event = OutreachEvent(
            ...
            type="manual",
            provider="   ",
            payload={"provider": "  SeNdGrId  ", "provider_event_id": "sg-payload-filter-blank-1"},
            created_at=datetime.now(timezone.utc),
        )
        self.db.add(legacy_event)
        self.db.commit()
    ...
    def test_list_suppression_normalizes_legacy_url_rows(self) -> None:
        from app.models import Suppression

        self._seed_lead_bundle()
        self.db.add(Suppression(email_or_domain=" https://LeGaCy.ExAmPlE/path ", reason="opt_out"))
        self.db.commit()
