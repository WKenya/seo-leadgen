...
- Status note (2026-03-15): worker discovery now prebuilds a normalized hostname lookup for legacy `website_url`/`website_domain` fallback matching, avoiding repeated per-item full scans during a batch.
...
Status note (2026-03-16): webhook ingest now lazily builds domain fallback lookup only when `email_or_domain` resolution is needed, caches normalized `email_or_domain` lead matches per payload, dedupes suppression upserts per payload, and batch-prefetches trimmed `external_id` values once per request to reduce per-event duplicate-id/lead-resolution/suppression queries while preserving duplicate/rejection semantics.
